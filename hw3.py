...
def encrypt(plaintext, prime, exp, root):
    ciphertext = []
...
def decrypt(ciphertext, prime, privKey, root):
    plaintext = []
...
if __name__ == '__main__':
    ...
    root = random.randint(2, prime)

    privKey = generateKey(prime)
    exp = powerMod(root, privKey, prime)

    message = input("Enter a message to encrypt: ")
    print("Original message: ", message)

    ciphertext, pubKey = encrypt(message, prime, exp, root)
    print("Encrypted message: ", ciphertext)

    decryption = decrypt(ciphertext, pubKey, privKey, prime)
